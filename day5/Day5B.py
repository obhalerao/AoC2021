...
for i in range(len(vlines)):
    if(vlines[i][0][0] == vlines[i][1][0]):
        ...
        l1 = min(vlines[i][0][0], vlines[i][1][0])
        r1 = max(vlines[i][0][0], vlines[i][1][0])
        l2 = min(vlines[i][0][1], vlines[i][1][1])
        r2 = max(vlines[i][0][1], vlines[i][1][1])
        range1 = None
        range2 = None
        if(vlines[i][0][0] > vlines[i][1][0]):
            range1 = range(vlines[i][0][0], vlines[i][1][0]-1, -1)
        else:
            range1 = range(vlines[i][0][0], vlines[i][1][0]+1)

        if (vlines[i][0][1] > vlines[i][1][1]):
            range2 = range(vlines[i][0][1], vlines[i][1][1] - 1, -1)
        else:
            range2 = range(vlines[i][0][1], vlines[i][1][1] + 1)
